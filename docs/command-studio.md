...
5. Click **Edit buttons** and create a local test action: `/echo Potato bridge test`, wait 1500 ms, then `/echo Second step`. Apply & save, then **Done editing**. Click the action in the clean button view. Confirm each line appears once, in order, only in the chosen client's chat, about 1.5 seconds apart. This tests dispatch, not a band broadcast.
...
- The studio always opens in **button view**: only client selection, sequence Stop, group navigation and action buttons are shown. **Edit buttons** reveals the tree, customization toolbar and property editor. **Done editing** returns to button view, offering Save, Discard or Cancel for pending edits. Your origin-client selection and current group are preserved.
- In editor mode, clicking an action only selects it for editing; it never sends commands. Group clicks still navigate. Stop any running sequence before switching modes. Right-clicking a tile in button view is an explicit shortcut into editor mode for that button.
...
- In editor mode, the left tree selects buttons to edit; double-click a group to open it. Back/Home navigate without sending anything. Action tiles execute only in button view.
...
- The editor changes labels, game icon IDs, custom artwork, background/text colors, parent, action/group behavior, and command steps. **Apply & save button** commits edits. Tile size applies to the entire studio. Navigation and hover/press feedback are immediate: there are no studio animation timers, moving transitions or Animate setting. Old profiles with an Animate field still load; that obsolete field is ignored. Command-step waits are unaffected.
